...
//				alert("remove child markup for " + node);
	...
	nodeRenderTitle: function(ctx, title) {
		...
		if(!nodeTitle){
			...
			var fathernode =node.getParent(); //recupère le parent
			var prioritychild = "false";
			var priorityparent = "false";
			var prioritychild = fathernode.data.priority; //recupere la priorité du fils Kevin Script
			var priorityparent = node.data.priority; //recupere la priorité du node actuel
			//alert("node actuel : "+node+ " node parent : "+importance);
			if(prioritychild == "true" || priorityparent == "true"){ nodeTitle = "<span " + role + " class='fancytree-title priority'" + id + tooltip + tabindex + ">" + node.title + "</span>";}
			else{ nodeTitle = "<span " + role + " class='fancytree-title '" + id + tooltip + tabindex + ">" + node.title + "</span>";}
		}
		ares.push(nodeTitle);
