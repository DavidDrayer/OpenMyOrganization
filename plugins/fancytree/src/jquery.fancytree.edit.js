...
$.ui.fancytree._FancytreeNodeClass.prototype.editEnd = function(applyChanges, _event){
	...
	if( doSave ) {
		var savekey = node.key;
		var tabsavekey = savekey.split('-');
		var typesave = 0; 
		for(var a=0;a<tabsavekey.length;a++){  //on determine le type de la key
        typesave++; 
		}
		switch( typesave ) {
		  //Niveau 1 ne peut pas être renomé (rôles)	
		  case 2: //on va drop au niveau d'un projet, item, ou action unique
		  var namekeysave = tabsavekey[1];
		  var lettresave = namekeysave.substring(0,1);
		 switch( lettresave ){
				case "p": //Renome le projet
				var idprojet = namekeysave.substring(1);
                var dataTab = {
				"action": "EditProject",
				"ID": idprojet,
				"title": newVal
				...
				case "i": //Renome le item
				var iditem = namekeysave.substring(1);
				var dataTab = {
				"action": "EditItemBox",
				"ID": iditem,
				"title": newVal
				...
				case "a": //Renome l'action unique
				var idaction = namekeysave.substring(1);
		...
		node.renderTitle();
		//alert("Dans jquery.fancytree.edit.js => code si la valeur titre est non changé ou New Item");
	}
