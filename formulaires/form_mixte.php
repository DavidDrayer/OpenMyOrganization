<?
	...
	if (isset($_GET["action"])) {
		...
		if ($_GET["action"]=="getForm") {
			...
			echo "<input type='hidden' id='form_target' value='/formulaires/form_mixte.php'/>";
...
<script>
  ...
  beforeLoad: function( event, ui ) {
    ...
    function addTab(no,text) {
      var label = text,
        id = tabCounter,
        li = $( tabTemplate.replace( /#\{href\}/g, "/formulaires/form_mixte.php?circle=<?=$_POST["circle"]?>&action=getForm&param="+no+"&id=" + id ).replace( /#\{label\}/g, label ).replace( /#\{id\}/g, "tab_gouv_"+id ) );
...
?>
  <div id="tabs_gouv">
  <ul>
    <li><a id="tab_gouv_1" href="/formulaires/form_mixte.php?circle=<?=$_POST["circle"]?>&action=getForm&param=<?=$_POST["param"]?>&id=1"><?=utf8_decode($_POST["param_txt"])?></a></li>
  </ul>
</div>
...
<div id="new_action" title="Ajouter une action">
		...
		<p><button  name='new_action' class='new_action' value='13'>Ajouter une tension de gouvernance</button></p>
...
<script>
	...
	$("#formulaire").load('/formulaires/form_mixte.php?action=new&<?
