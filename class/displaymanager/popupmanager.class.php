<?php
	...
	class PopupManager implements GenericGraphicManager
	{
		...
		public function getBrowser($object) {
				...
				if(isset($object[0])){
					switch (get_class($object[0]))			
					{
						...
						default:
							...
							echo "Aucun browser popup pour les tableaux de classe ".get_class($object[0]);
					}
				}
				if(isset($object[1])){
			...
			switch (get_class($object))			
			{
				case "holacracy\\User" : 
					...
					return new \widget\wg_userViewer($object);
					break;
				default:
					// Aucun browser défini pour cet objet
					echo "Aucun browser popup pour les objets de classe -".get_class($object)."-";
			}
			}	
		}
