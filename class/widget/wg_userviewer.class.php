...
class wg_userViewer extends Widget
{
	...
	<?php
		...
		if (isset($_GET["organisation"]) || isset($_GET["circle"]) || isset($_GET["role"])) {
			if (isset($_GET["organisation"])) {
				...
				$roles=$this->_user->getRoles($this->_user->getManager()->loadOrganisation($_GET["organisation"]),\holacracy\Role::STANDARD_ROLE | \holacracy\Role::LINK_ROLE | \holacracy\Role::STRUCTURAL_ROLES);
				$myroles=$_SESSION["currentUser"]->getRoles($this->_user->getManager()->loadOrganisation($_GET["organisation"]),\holacracy\Role::STANDARD_ROLE | \holacracy\Role::LINK_ROLE | \holacracy\Role::STRUCTURAL_ROLES);
